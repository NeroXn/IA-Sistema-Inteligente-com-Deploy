...
with tab1:
    ...
    st.markdown("**Opções de treino**")
    col_a, col_b, col_c = st.columns(3)
    with col_a:
        do_oversample = st.checkbox("Balancear classes (oversample minority)", value=False)
    with col_b:
        do_tune = st.checkbox("Hiperparam tuning (mais tempo)", value=False)
    with col_c:
        n_estimators = st.number_input("n_estimators (padrão)", min_value=50, max_value=1000, value=200, step=50)

    # safe load/train logic
    pipe_clf = None
    if os.path.exists(CLASS_MODEL_PATH):
        try:
            pipe_clf = joblib.load(CLASS_MODEL_PATH)
            st.success("Modelo de classificação carregado.")
        except Exception as e:
            st.warning("Falha ao carregar modelo salvo, será re-treinado: " + str(e))
            pipe_clf = None
    if pipe_clf is None:
        # prepare data
        X = df[FEATURES]
        ...
        if do_oversample:
            ...
            st.info(f"Dados balanceados por oversample: tamanho {len(y)}")
        preproc = build_preprocessor(X)
        clf = RandomForestClassifier(n_estimators=int(n_estimators), random_state=42, n_jobs=-1, class_weight="balanced")
        pipe_temp = Pipeline([("preproc", preproc), ("clf", clf)])
        X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.25, stratify=y, random_state=42)
        if do_tune:
            st.info("Executando RandomizedSearchCV (n_iter=8)")
            param_dist = {
            ...
            rnd = RandomizedSearchCV(pipe_temp, param_dist, n_iter=8, cv=3, random_state=42, n_jobs=1, verbose=0)
            rnd.fit(X_train, y_train)
            pipe_clf = rnd.best_estimator_
            st.write("Melhor conjunto de parâmetros:", rnd.best_params_)
        else:
            pipe_temp.fit(X_train, y_train)
            pipe_clf = pipe_temp
        joblib.dump(pipe_clf, CLASS_MODEL_PATH)
    ...
    plot_feature_importance(pipe_clf, X_test, title="Importância das features (Classificação)")

    st.markdown("---")
    st.subheader("Prever para novos clientes (upload CSV)")
    st.info("Envie um CSV com as mesmas colunas do dataset German Credit (veja README).")
    uploaded = st.file_uploader("CSV (1 linha ou mais) — Classificação", type=["csv"], key="clf_csv")
    if uploaded is not None:
        try:
            input_df = pd.read_csv(uploaded)
            # ensure all columns exist; fill missing with mode/median
            for c in FEATURES:
                if c not in input_df.columns:
                    input_df[c] = df[c].mode().iloc[0] if df[c].dtype == "O" else df[c].median()
            X_input = input_df[FEATURES]
            preds = pipe_clf.predict(X_input)
            probas = pipe_clf.predict_proba(X_input)[:,1]
            out = input_df.copy()
            out["pred_label"] = np.where(preds==1, "ALTO_RISCO", "BAIXO_RISCO")
            out["prob_mau"] = probas
            st.dataframe(out.head())
        except Exception as e:
            st.error("Erro ao processar CSV de classificação: " + str(e))
